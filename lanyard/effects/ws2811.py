...
class Controller:
    ...
    # Has a bug where one pixel isn't filled.  
    def fill_random():
        Controller.fill_black()
        used = [0] * num_pixels
        lights = 0

        while( lights < num_pixels - 1 ):
    ...
    def wheelBrightLevel(pos, bright):
        ...
        # bight level logic
        color = Controller.brightnessRGB(r, g, b, bright)
        r = color[0]
        g = color[1]
        b = color[2]
    ...
    def candycane_custom(c1, c2, thisbright, delay, cycles):
        ...
        for loop in range(cycles):
            ...
            for i in range(N6):
                ...
                #print(j0)
                pixels[j0] = Controller.brightnessRGB(c1[0], c1[1], c1[2], int(thisbright*.75))
                pixels[j1] = Controller.brightnessRGB(c2[0], c2[1], c2[2], thisbright)
                pixels[j2] = Controller.brightnessRGB(c1[0], c1[1], c1[2], int(thisbright*.75))
                pixels[j3] = Controller.brightnessRGB(c2[0], c2[1], c2[2], thisbright)
                pixels[j4] = Controller.brightnessRGB(c1[0], c1[1], c1[2], int(thisbright*.75))
                pixels[j5] = Controller.brightnessRGB(c2[0], c2[1], c2[2], thisbright)
        ...
        for loop in range(cycles):               
            ...
            for ii in range(1, 252, beat1Step): #for ( ii = 1 ; ii <252 ; ii = ii = ii + x)
                for index in range(num_pixels):
                    ...
                    pixels[index] = Controller.brightnessRGB(r,g,b, ii) 
    ...
    def randomLevelsCustom( levelobj, clearall, delay, cycles ):
        ...
        for loop in range(cycles):
            ...
            if (NUM_LEVELS == level):
                level = 0
            Controller.light_level_random(levelobj, level, 1)
            pixels.show()
            time.sleep(delay)
    ...
    def randomLevelsCustomColors( colorobj, levelobj, clearall, delay, cycles ):
        ...
        for loop in range(cycles):
            ...
            if (NUM_LEVELS == level):
                level = 0
            Controller.light_level_random_color(pcolor, levelobj, level, 1)
            pixels.show()
            time.sleep(delay)
    ...
    def randomLevelsCustom2Colors( c1, c2, levelobj, clearall, delay, cycles ):
        ...
        for loop in range(cycles):
            ...
            if (NUM_LEVELS == level):
                level = 0
            Controller.light_level_random_color(color, levelobj, level, 1)
            pixels.show()
            time.sleep(delay)
    ...
    def LevelsColorsCustom( colorobj, levelobj, delay ):
        ...
        for levelnum in range(NUM_LEVELS):
            ...
            if (NUM_LEVELS == levelnum):
                ...
                level = levelnum
            Controller.light_level_random_color(pcolor, levelobj, level, False)
            pixels.show()
            time.sleep(delay)
